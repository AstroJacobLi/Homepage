...
这样，就可以利用全部时间序列的数据拟合出一个$$\gamma, \ K,\ \phi$$出来。给定后两个参数，然后对4 epochs的数据再去拟合$$\gamma$$，得到的结果见上图。发现$$\gamma$$会有一个Long period variation，于是猜测该系统会不会有第三个planet？但是之后更高精度的观测否定了这个$$\gamma$$ -velocity的猜测（Subo说的）。
